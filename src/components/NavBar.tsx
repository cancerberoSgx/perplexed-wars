  ...
  public render() {
    return (
      <nav className="navbar navbar-light bg-light">
        ...
        <button className="navbar-toggler" type="button" onClick={toggleCollapseHandler} data-target="#navbarTogglerDemo01" aria-controls="navbarTogglerDemo01" aria-expanded="false" aria-label="Toggle navigation">
        ...
        <div className="collapse navbar-collapse" id="navbarTogglerDemo01">
          ...
          <ul className="navbar-nav mr-auto mt-2 mt-lg-0">
            ...
            <li className="nav-item">
            <span>Interval: <input type="number" className="intervalInput" onChange={changeInterval} defaultValue={this.state.game.interval + ''}/><button className="btn decrement" onClick={decrementInterval}>-</button><button className="btn increment" onClick={incrementInterval}>+</button></span>
            </li>
            <li className="nav-item">
...
function incrementInterval() {
  doChangeInterval(State.get().game.interval + 100)
}
function decrementInterval() {
  doChangeInterval(State.get().game.interval - 100)
}
function changeInterval(e:React.ChangeEvent<HTMLInputElement>) {
  const interval = parseInt(e.currentTarget.value, 10)  // TODO: validation and throttle
  doChangeInterval(interval)
}
function doChangeInterval(interval) {
  if (interval >= 200) {
    console.log('doChangeInterval', interval)
    store().dispatch({
